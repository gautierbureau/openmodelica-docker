...
# Out: return code 0 -> True
#      return code 1 -> False
image_with_tag_exists() {
  if [ -z "$1" ]; then
    echo "image_exists needs a name of image as argument."
    exit 1
  fi
  local test_image_name=$(docker images --format "{{.Repository}}:{{.Tag}}" | sort | uniq  | grep -w "^${1}$")
  if [ "$test_image_name" ]; then
    return 0
  else
    return 1
...
delete_image_with_tag() {
  if [ -z "$1" ]; then
    echo "delete_image needs a name of image as argument."
    exit 1
  fi
  local image_name=$1
  image_with_tag_exists $image_name
  if `image_with_tag_exists $image_name`; then
    if ! `container_exists_for_image $image_name`; then
      docker rmi $image_name
    else
      echo "Some containers are still using the image:"
    ...
    echo "You specified an image name that is not existing."
    echo "List of available images:"
    for name in `docker images --format "{{.Repository}}:{{.Tag}}" | sort | uniq`; do echo "  $name"; done
    exit 1
  fi
}
